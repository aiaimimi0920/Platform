...
- Heavy Chat read paths must follow `docs/40-engineering/heavy-chat-read-query-baseline.md`: snapshot hydration batches slot-agent bindings, slot-project bindings, and all owned thread messages in owner-scoped queries, while Gateway history uses its own pre-sequence complete-message projection. UI pagination must remain separate from Gateway context and must not silently truncate either contract.
