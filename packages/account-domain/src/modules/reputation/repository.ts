...
export async function getReputationTaskStats(
  ...
  if (env.usesDedicatedDatabase) {
    const platformSummary = await getCorePlatformSummary(userId);
    return {
      completedTaskCount: platformSummary?.reputationStats.completedTaskCount ?? 0,
      defaultedTaskCount: platformSummary?.reputationStats.defaultedTaskCount ?? 0,
      cancelledTaskCount: platformSummary?.reputationStats.cancelledTaskCount ?? 0,
      activeTaskCount: platformSummary?.reputationStats.activeTaskCount ?? 0,
      favorableArbitrationCount: platformSummary?.reputationStats.favorableArbitrationCount ?? 0,
      unfavorableArbitrationCount: platformSummary?.reputationStats.unfavorableArbitrationCount ?? 0,
