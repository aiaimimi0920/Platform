...
- `P2-CI-001` 已核销：三条 GitHub Actions workflow 的全部第三方 Action 已从可移动 major tag 固定到对应官方 tag 当日解析并复核的完整 40 位 commit SHA；repository contract 同时拒绝未登记 Action、非完整 SHA、与审计 pin 不一致或缺失版本注释的引用。
