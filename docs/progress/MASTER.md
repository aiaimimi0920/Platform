...
- [ ] Phase 4: 核心领域测试与安全可观测性 (3/4 tasks) [details](./phase-4-domain-security-observability.md)
...
- 当前动作：`P4-03` 已完成并把 required integration fixture、workspace `test:integration:required` contract、OAuth callback/state/account-linking test 接入 Platform 门禁；下一步进入 `P4-04`，补 correlation id、dependency error taxonomy 与 secret redaction。
...
1. 完成 `P4-04`，补齐 correlation id、dependency error taxonomy 与 secret redaction。
