...
import {
  ...
  evaluateGatewayRemediationEffectivenessAnomalyMailboxRoutePolicy,
  gatewayRemediationEffectivenessAnomalyMailboxRouteName,
} from "./gateway-anomaly-remediation-effectiveness-alert-routing";

describe("gateway remediation effectiveness anomaly mailbox routing", () => {
  it("builds a stable mailbox cadence route config", () => {
    const route = buildGatewayRemediationEffectivenessAnomalyMailboxRouteConfig({
      cooldownMinutes: 180,
      maxDeliveriesPerIncident: 6,
    });

    assert.equal(route.name, gatewayRemediationEffectivenessAnomalyMailboxRouteName);
    assert.deepEqual(route.eventNames, ["aiGateway.remediationEffectivenessAnomalyAlerted"]);
  ...
  it("blocks mailbox delivery during cooldown and after delivery cap", () => {
    ...
    const cooldownDecision = evaluateGatewayRemediationEffectivenessAnomalyMailboxRoutePolicy({
      cooldownMinutes: 180,
      maxDeliveriesPerIncident: 6,
    ...
    const capDecision = evaluateGatewayRemediationEffectivenessAnomalyMailboxRoutePolicy({
      cooldownMinutes: 180,
