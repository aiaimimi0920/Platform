...
async function requireModulesEnabled(_moduleKeys: string[]): Promise<boolean> {
  return true;
}

const insertMailboxMessage = createMailboxMessage;

export async function dispatchMailboxOpsCampaigns() {
  return dispatchDueMailboxOpsCampaigns(10);
}

async function refreshProductShadowFromEvent(productId: string, eventName: string) {
  if (!productId) return;

  try {
    await db.transaction(async (tx) => {
      await refreshProductSnapshotInTx({
        tx,
        productId,
      });
    });
    ...
    console.warn(
      `[account-worker] failed to refresh product shadow for ${eventName} ${productId}: ${
...
async function deleteProductShadowFromEvent(productId: string, eventName: string) {
  if (!productId) return;

  try {
    await db.transaction(async (tx) => {
      await deleteProductSnapshotInTx({
        tx,
        productId,
      });
    });
    ...
    console.warn(
      `[account-worker] failed to delete product shadow for ${eventName} ${productId}: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
...
async function syncGatewayAccessGrantFromCore(itemId: string, eventName: string) {
  ...
  try {
    ...
    console.warn(
      `[account-worker] failed to sync gateway access grant for ${eventName} ${itemId}: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
...
const handlers: Record<EventName, EventHandler> = {
  ...
  "email.delivery.requested": async (payload) => {
    const jobId = String(payload.jobId || "").trim();
    if (!jobId) return "processed";
    await deliverEmailJob(jobId);
    return "processed";
  },
  "email.inbound.received": async (payload) => {
    const providerInboundMessageId = String(payload.providerInboundMessageId || "").trim();
    if (!providerInboundMessageId) return "processed";
    await processEmailProviderInboundMessage(providerInboundMessageId);
    return "processed";
  },
  "dailyReward.claimed": async (payload) => {
    // Routine sign-in settlement stays in local mission UI/toast feedback and should not enter mailbox.
    return "processed";
  },
  "mission.claimed": async (payload) => {
    // Routine mission settlement stays in the mission center and top-left toast stack.
    return "processed";
  },
  "dailyMission.claimed": async (payload) => {
    // Legacy routine reward-mail path disabled. Controlled campaign rewards should be sent explicitly.
    return "processed";
  },
  ...
  "agentExecution.callbackRemediationAlerted": async (payload) => {
    if (!(await requireModulesEnabled(["agentExecution"]))) return "processed";
    ...
    const mailboxEnabled = await requireModulesEnabled(["mailbox"]);
    ...
    if (mailboxEnabled && agentOwnerUserId && !env.platformOperatorUserIds.includes(agentOwnerUserId)) {
      await insertMailboxMessage({
    ...
    await sendNotificationWebhook("agentExecution.callbackRemediationAlerted", payload);
  ...
  "agentExecution.runtimePressureAlerted": async (payload) => {
    ...
    const claimableQueuedExecutionCount = Number(payload.claimableQueuedExecutionCount || 0);
    const blockedQueuedExecutionCount = Number(payload.blockedQueuedExecutionCount || 0);
    const blockedByProfileCount = Number(payload.blockedByProfileCount || 0);
    const blockedByOwnerCount = Number(payload.blockedByOwnerCount || 0);
    ...
    const body =
      ...
      `claimable=${claimableQueuedExecutionCount}，blocked=${blockedQueuedExecutionCount}，` +
      `profile-blocked=${blockedByProfileCount}，owner-blocked=${blockedByOwnerCount}。` +
      `${detail ? ` ${detail}` : ""}`;
    ...
    if (mailboxEnabled && ownerUserId && !env.platformOperatorUserIds.includes(ownerUserId)) {
      await insertMailboxMessage({
        userId: ownerUserId,
        title: "你的 runtime profile 需要关注",
        body: `你在 runtime profile ${profileKey} 上出现并发压力：L${alertLevel} / ${schedulingDecisionClass} / queued=${queuedExecutionCount} / blocked=${blockedQueuedExecutionCount}。请前往 operator callback 面板查看 runtime pressure。`,
        type: "system",
      });
    }

    await sendNotificationWebhook("agentExecution.runtimePressureAlerted", payload);

    return "processed";
  },
  "aiGateway.anomalyIncidentAlerted": async (payload) => {
    const mailboxEnabled = await requireModulesEnabled(["mailbox"]);
    const alertLevel = Number(payload.alertLevel || 0);
    const projectId = String(payload.projectId || "").trim();
    const incidentId = String(payload.incidentId || "").trim();
    const anomalyCode = String(payload.reasonCategory || "unknown").trim();
    const followUpStatus = String(payload.followUpStatus || "pending").trim();
    const policyId = String(payload.policyId || "none").trim();
    const ownerUserId = String(payload.ownerUserId || "").trim();
    const count = Number(payload.count || 0);
    const detail = String(payload.detail || "").trim();
    const operatorTitle =
      alertLevel >= 3 ? "AI Gateway 异常需要紧急处理" : "AI Gateway 异常进入观察态";
    const body =
      `AI gateway anomaly 命中 L${alertLevel}，project=${projectId || "global"}，incident=${incidentId || "none"}，` +
      `code=${anomalyCode}，followUp=${followUpStatus}，policy=${policyId}，syncHits=${count}。` +
      `${detail ? ` ${detail}` : ""}`;
    ...
    await sendNotificationWebhook("aiGateway.anomalyIncidentAlerted", payload);

    return "processed";
  },
  "aiGateway.remediationEffectivenessAnomalyAlerted": async () => {
    // This alert path is dispatched directly from the remediation effectiveness anomaly snapshot loop.
    // Keep an explicit no-op handler so EventName remains exhaustively covered without duplicating delivery.
    return "processed";
  },
  "aiGateway.rateLimitHotspotAnomalyAlerted": async () => {
    // This alert path is dispatched directly from the rate-limit hotspot anomaly snapshot loop.
    // Keep an explicit no-op handler so EventName remains exhaustively covered without duplicating delivery.
    return "processed";
  },
  ...
  "reputation.updated": async (payload) => {
    if (!(await requireModulesEnabled(["reputation"]))) return "processed";

    const userIds = normalizeReputationUpdatedUserIds(payload);
    if (userIds.length === 0) return "processed";

    await db.transaction(async (tx) => {
      await refreshReputationUsersInTx(tx, userIds);
    });
  ...
  "product.updated": async (payload) => {
    ...
    await refreshProductShadowFromEvent(productId, "product.updated");
    return "processed";
  },
  "product.deactivated": async (payload) => {
    const productId = String(payload.productId || "");
    await refreshProductShadowFromEvent(productId, "product.deactivated");
    return "processed";
  },
  "product.deleted": async (payload) => {
    const productId = String(payload.productId || "");
    await deleteProductShadowFromEvent(productId, "product.deleted");
    return "processed";
  },
  "product.orderRolledBack": async (payload) => {
    if (!(await requireModulesEnabled(["product"]))) return "processed";
    const itemIds = Array.isArray(payload.itemIds)
      ? payload.itemIds.map((itemId) => String(itemId || "").trim()).filter(Boolean)
      : [];

    for (const itemId of itemIds) {
      await syncGatewayAccessGrantFromCore(itemId, "product.orderRolledBack");
    }
  ...
  "product.purchased": async (payload) => {
    ...
    if (productId) {
      await refreshProductShadowFromEvent(productId, "product.purchased");
    }

    return "processed";
  },
  "item.granted": async (payload) => {
    const itemId = String(payload.itemId || "").trim();
    if (!itemId) return "processed";
    await syncGatewayAccessGrantFromCore(itemId, "item.granted");
    return "processed";
  },
