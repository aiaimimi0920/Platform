...
RUN npm run build --workspace @neuro/account-worker
...
COPY --from=build --chown=node:node /app/services/account-worker /app/services/account-worker

WORKDIR /app/services/account-worker
USER node
EXPOSE 7303
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 CMD ["node", "-e", "fetch('http://127.0.0.1:7303/ready').then(r=>{if(!r.ok)process.exit(1)}).catch(()=>process.exit(1))"]
CMD ["npm", "run", "start"]
