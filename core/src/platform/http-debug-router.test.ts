...
function restoreEnv() {
  process.env.DATABASE_URL = originalDatabaseUrl ?? "postgres://core-test";
  process.env.REDIS_URL = originalRedisUrl ?? "redis://core-test";
...
describe("core platform HTTP debug router", () => {
  afterEach(() => {
    restoreEnv();
  });

  it("exposes CORS observability only to internal platform operators", async () => {
    process.env.DATABASE_URL = "postgres://core-test";
    process.env.REDIS_URL = "redis://core-test";
    ...
    const [
      ...
      { platformCorsOrigin, resetPlatformCorsObservabilityForTests, serializePlatformError },
      { HttpError },
    ] = await Promise.all([
      import("./http-debug-router"),
      import("@/platform/http-server"),
      import("@/platform/errors"),
    ]);
